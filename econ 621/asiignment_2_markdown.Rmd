...
#======================#
  ...
  # laod data 
  dt <- data.table(
    read.dta("c://users/Nmath_000/Documents/MI_school/Second Year/621 Labor/Assignments/Assignment_2/MROZ.DTA"))
...
#===============================#
  ...
  var_l <- c("age", "educ", "kidslt6", "kidsge6",
             "husage", "huseduc", "wage", "huswage",
             "nonlab_i", "hours", "hushrs"  )
  
  tb1 <- melt.data.table(round(dt[,lapply(.SD,mean), .SDcols =var_l],2), 
                         value.name = "Full Sample Mean")
  
  tb2 <- melt.data.table(round(dt[,lapply(.SD,sd), .SDcols =var_l],2), 
                         value.name = "Full Sample Standard Deviation")
  
  tb3 <- melt.data.table(round(dt[inlf == 1,lapply(.SD,mean), .SDcols =var_l],2),
                         value.name = "Working Women Mean")
  
  tb4 <- melt.data.table(round(dt[inlf == 1,lapply(.SD,sd), .SDcols =var_l],2),
                         value.name = "Working Women Standard Deviation")
...
#======================#
  ...
  vars <- c("age", "educ", "age_sq", "age_cu", 
            "educ_sq", "educ_cu", "age_educ", "age_sq_educ", 
            "age_educ_sq", "unem",  "city", "motheduc", 
            "fatheduc", "IMR")
...
save_tex_tables <- function(letter = NULL){
  ...
  print(xtable(table, type = "latex"), 
        file = paste0("C:/Users/Nmath_000/Documents/Code/courses/econ 621/assignment_2_tex/t_",
                      letter, ".tex"),
