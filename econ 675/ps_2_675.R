  ...
  #=============================#
    ...
    imse_f <- function(in_data, x_v, h_v = NULL, f_x = f_x){
      ...
      for(i in 1:length(h_v)){
        ...
        # Compute imse 
         fhat_li <- sapply(as.matrix(in_data),function(x) 1/(n*h)*sum(K0((as.matrix(in_data)-x)/h))) 
       
       # Compute imse 
         fhat_lo <- sapply(1:n,function(i) 1/(n*h)*sum(K0((as.matrix(in_data)[-i]-as.matrix(in_data)[i])/h))) 
       
        # put into data.table 
         in_data[,fhat_li := fhat_li ]
         in_data[,fhat_lo := fhat_lo ]
  
        # now add in f_x for each 
        in_data[, f_x_li := f_x(get(x_v))]
        in_data[, f_x_lo := f_x(get(x_v))]
        
        # now do squared error 
        in_data[, sq_er_li := (fhat_li - f_x_li)^2]
        in_data[, sq_er_lo := (fhat_lo - f_x_lo)^2]
        
        # now get imse 
        imse_li <- in_data[, mean(sq_er_li)]
        imse_lo <- in_data[, mean(sq_er_lo)]
